...
t2_ez_fa_fit_t2_demog_pred = read.csv(paste0(input_path,'/prediction/pred_out_ez_t2_fa_3_scores_demog_fa_scores_t2_pred.csv'))

t2_ez_fa_fit_t2_demog_pred = t2_ez_fa_fit_t2_demog_pred %>%
...
t2_ez_mes_t2_demog_pred = read.csv(paste0(input_path, 'prediction/pred_out_res_clean_retest_data_ez_demog_fa_scores_t2_pred.csv'))

t2_ez_mes_t2_demog_pred = t2_ez_mes_t2_demog_pred %>%
...
t2_raw_t2_demog_pred = read.csv(paste0(input_path,'prediction/pred_out_res_clean_retest_data_raw_demog_fa_scores_t2_pred.csv'))

t2_raw_t2_demog_pred = t2_raw_t2_demog_pred %>%
...
p = t2_ez_fa_fit_t2_demog_pred %>%
  bind_rows(t2_ez_mes_t2_demog_pred) %>%
  bind_rows(t2_raw_t2_demog_pred) %>%
...
ggsave(paste0('t2_fit_pred_9.', out_device), plot=p, device = out_device, path = fig_path, width = 20, height = 9, units = "in")

###############################

t2_ez_fa_pred_t2_demog_pred = read.csv(paste0(input_path,'/prediction/pred_out_ez_t2_fa_3_pred_scores_demog_fa_scores_t2_pred.csv'))

t2_ez_fa_pred_t2_demog_pred = t2_ez_fa_pred_t2_demog_pred %>%
  mutate(RsquaredSE = RsquaredSD/sqrt(10),
         iv=as.character(iv),
         dv = as.character(dv),
         iv_data = as.character(iv_data),
...
ggsave(paste0('t2_pred_pred_9.', out_device), plot=p, device = out_device, path = fig_path, width = 20, height = 9, units = "in")

###############################

# ggsave(paste0('t1_fit_pred_4.', out_device), plot=p, device = out_device, path = fig_path, width = 20, height = 9, units = "in")
