...
you could look at this with factor analysis but there are more variables than observations. so first i'll just explore correlations  

Correlate each DDM measure with  
- raw measures within task  
- other ddm measures across tasks  

```{r}
tmp = data.frame(cor(test_data_std, use="complete.obs"))
tmp = tmp %>%
  mutate(dv = row.names(.)) %>%
  gather(key, value, -dv) %>%
...
tmp = tmp %>%
  ...
  mutate(task_task = ifelse(task_group_1 == task_group_2 & (ddm_raw_1 == "ddm" | ddm_raw_2 == "ddm"), "task-task", ifelse(task_group_1 != task_group_2 & (ddm_raw_1 == "ddm" | ddm_raw_2 == "ddm"), "ddm-ddm",NA)))
...
tmp %>%
  ...
  geom_histogram()+
  geom_vline(data=tmp_med, aes(xintercept=median_abs_cor), color="red", linetype = "dashed")+
  facet_wrap(~task_task)+
...
```{r}
summary(lmer(abs(value)~task_ddm+(1|var_1), tmp))
```
...
fit <- prcomp(test_data_std %>% na.omit())

summary(fit)
plot(fit, type = "lines")

fit <- principal(test_data_std, nfactors=3, rotate="varimax")
fit
