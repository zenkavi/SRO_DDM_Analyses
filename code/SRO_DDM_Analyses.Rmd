...
Can we recover a 3 factor structure for 
-EZ and
-HDDM variables that captures comparable processes across tasks?
...
```{r}
clean_test_data_ez = clean_test_data %>%
  select(grep('EZ', names(clean_test_data_std), value=T))

clean_test_data_hddm = clean_test_data %>%
  select(grep('hddm', names(clean_test_data_std), value=T))
```

Factor analysis on T1 EZ data

```{r}
ez_fa = fa(clean_test_data_ez, 3, rotate='oblimin', fm='minres', n.obs=150, scores='tenBerge')
```

```{r}
ez_loadings = as.data.frame(ez_fa$loadings[])

ez_loadings[abs(ez_loadings)<0.3]=NA
