...
test_data <- test_data[,c(1,names(test_data) %in% measure_labels$dv)]
...
retest_data <- retest_data[,c(1,names(retest_data) %in% measure_labels$dv)]
...
hddm_refits = hddm_refits[,c(1,names(hddm_refits) %in% measure_labels$dv)]
...
test_data_retest_sample_hddm = cbind(test_data$sub_id, test_data[,names(test_data) %in% names(hddm_refits) == FALSE])

#fix naming before merging
names(test_data_retest_sample_hddm)[which(names(test_data_retest_sample_hddm) == 'test_data$sub_id')] <-'sub_id'

#merge hddm refits to test data
test_data_retest_sample_hddm = merge(test_data_retest_sample_hddm, hddm_refits, by="sub_id")
