...
test_data <- test_data[,c(names(test_data) %in% measure_labels$dv)]
...
retest_data <- retest_data[,c(names(retest_data) %in% measure_labels$dv)]
...
#drop hddm columns from test_data with hddm full sample
test_data_hddm_retest_sample = cbind(test_data_hddm_full_sample$sub_id, test_data_hddm_full_sample[,names(test_data_hddm_full_sample) %in% names(hddm_refits) == FALSE])

tmp = names(test_data_hddm_full_sample)[names(test_data_hddm_full_sample) %in% names(hddm_refits) == FALSE]

###RERUN MOTOR SS FOR REFIT (MISSING REACTIVE CONTROL HDDM DRIFT)
tmp

test_data_hddm_full_sample %>%
  select()

#fix naming before merging
names(test_data_hddm_retest_sample)[which(names(test_data_hddm_retest_sample) == 'test_data_hddm_full_sample$sub_id')] <-'sub_id'

#merge hddm refits to test data
test_data_hddm_retest_sample = merge(test_data_hddm_retest_sample, hddm_refits, by="sub_id")

rm(test_data, hddm_refits)
```
