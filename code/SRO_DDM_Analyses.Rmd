...
rel_df %>%
  ...
  gather(key, val, -rt_acc, -contrast) %>%
  separate(key, into = c("tmp1", "tmp2", "type"), sep = "_", extra = "drop") %>%
  select(-tmp2) %>%
  spread(tmp1, val) %>%
  ggplot(aes(type, mean, color=rt_acc))+
  geom_point(position=position_dodge(width=0.5))+
  facet_wrap(~factor(contrast, levels=c("non-contrast", "contrast"), labels=c("Non-contrast", "Contrast")))

unique(rel_df$rt_acc)
```
