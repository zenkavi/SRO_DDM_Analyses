...
```{r echo=FALSE, out.width='100%'}
fig_name = 'HDDM_rel_150vs552.jpeg'
...
```{r}
tmp = refit_fitstats %>%
  select(m_log_rsq_adj, subj_id, task_name, sample) %>%
  left_join(t1_hierarchical_fitstats %>% select(m_log_rsq_adj, subj_id, task_name, sample), by = c("subj_id", "task_name")) 

tmp %>%
  ggplot(aes(m_log_rsq_adj.x, m_log_rsq_adj.y, col=task_name))+
  geom_smooth(method="lm")+
  geom_abline(slope=1, intercept=0, linetype="dashed")+
  theme(legend.position = "none")+
  xlab(expression(paste("Adjusted ", R^{2}, " n=150")))+
  ylab(expression(paste("Adjusted ", R^{2}, " n=552")))
```

```{r}
m = lmer(m_log_rsq_adj.x ~ m_log_rsq_adj.y+(1|task_name), tmp)
summary(m)
```

```{r}
compare_rsqs = function(data){
  m = lm(m_log_rsq_adj.x ~ m_log_rsq_adj.y, data)
  
  out = data.frame(rsq_coef = coef(summary(m))['m_log_rsq_adj.y', "Estimate"],
                   rsq_tval = coef(summary(m))['m_log_rsq_adj.y', "t value"])
  return(out)
}
...
tmp %>%
  ...
  group_by(task_name) %>%
  do(compare_rsqs(.)) %>%
  mutate(better_fit = ifelse(rsq_tval>2, '552', ifelse(rsq_tval<(-2), '150', 'no diff')))
```

```{r}
tmp = refit_fitstats %>%
  select(m_kl, subj_id, task_name, sample) %>%
  left_join(t1_hierarchical_fitstats %>% select(m_kl, subj_id, task_name, sample), by = c("subj_id", "task_name")) 

tmp %>%
  ggplot(aes(m_kl.x, m_kl.y, col=task_name))+
  geom_smooth(method="lm")+
  geom_abline(slope=1, intercept=0, linetype="dashed")+
  theme(legend.position = "none")+
  xlab("Mean KL for n=150")+
  ylab("Mean KL for n=552")
```

```{r}
m = lmer(m_kl.x ~ m_kl.y+(1|task_name), tmp)
summary(m)
```

```{r}
compare_kls = function(data){
  m = lm(m_kl.x ~ m_kl.y, data)
  
  out = data.frame(kl_coef = coef(summary(m))['m_kl.y', "Estimate"],
                   kl_tval = coef(summary(m))['m_kl.y', "t value"])
  return(out)
}
...
tmp %>%
  ...
  group_by(task_name) %>%
  do(compare_kls(.)) %>%
  mutate(better_fit = ifelse(kl_tval>2, '150', ifelse(kl_tval<(-2), '552', 'no diff')))
```
...
```{r  eval=FALSE, echo=FALSE}
rm(all_data_cor, all_data_cor_med, retest_data_cor, retest_data_cor_med, test_data_cor, test_data_cor_med)
```
