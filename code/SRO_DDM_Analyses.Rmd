...
test_data  = test_data_hddm_full_sample
...
rel_df = as.data.frame(matrix(nrow = length(numeric_cols), ncol = length(rel_df_cols)))
...
for(i in 1:length(numeric_cols)){
  rel_df[numeric_cols[i], 'icc'] <- get_retest_stats(numeric_cols[i])
...
```{r}
test_data  = test_data_hddm_retest_sample
```
