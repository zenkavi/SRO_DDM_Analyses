...
refit_fitstats %>%
  ...
  left_join(t1_hierarchical_fitstats, by = c("node", "task_name")) %>%
  ggplot(aes(mean_kl.x, mean_kl.y))+
  # geom_point(aes(color=task_name))+
  geom_smooth(method="lm",aes(color=task_name))+
...
The fit statistics for the same models using n=552 and n=150 for the priors are **barely** correlated and if anything the mean kl's for n=552 are slightly higher than the mean kl's for n=150 (accounting for differences between tasks).
...
#node/sub_id would have been the random slopes if you had fit_stats for each samples instead of the per subject summarized data
m = lmer(mean_kl.x ~ mean_kl.y+(1|task_name), tmp)
summary(m)
```

There aren't systematic differences between the kl's using n=552 or n=150 that generalizes across all tasks. In the two cases that there are systematic diferences the n=150 models are better fits.

```{r}
compare_kls = function(data){
  m = lm(mean_kl.x ~ mean_kl.y, data)
  
  out = data.frame(kl_coef = coef(summary(m))['mean_kl.y', "Estimate"],
                   kl_tval = coef(summary(m))['mean_kl.y', "t value"])
  return(out)
}
