...
```{r}
scatter3D(tmp$Dim.1, tmp$Dim.2, tmp$Dim.3, pch = 19, bty='g', 
          col.var = as.integer(factor(tmp$rt_acc)), 
          col = rt_acc_cols,
          colkey = list(side=1, #addlines=T,
                        at = c(-1,0,1),
                        length = 0.5, width = 0.5),
          theta = 90, phi = 145)

scatter3D(iris$Sepal.Length, iris$Petal.Length, iris$Sepal.Width, bty = "g", pch = 18, 
          col.var = as.integer(iris$Species), 
          col = c("#1B9E77", "#D95F02", "#7570B3"),
          pch = 18, ticktype = "detailed",
          colkey = list(#at = c(2, 3, 4), 
                        side = 1, 
          addlines = TRUE 
          #length = 0.5, width = 0.5
