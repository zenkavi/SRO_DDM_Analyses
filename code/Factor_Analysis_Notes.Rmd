...
```{r}
ez_measures = clean_test_data_std %>%
  select(sub_id, ez_vars)
ez_measures = merge(ez_measures, demog_data[,c("sub_id","Age", "Sex")], by="sub_id")
ez_measures

```

```{r}
demog_factors = names(demog_fa_scores)[-1]

out = data.frame(dv=NA, iv=NA, Rsquared=NA, RsquaredSD=NA)

ez_measures=ez_measures[-which(ez_measures$sub_id == "s202"),]
demog_fa_scores=demog_fa_scores[-which(demog_fa_scores$sub_id == "s202"),]

for(i in demog_factors_tmp){
  for(j in ez_vars){
...
ez_measures_pred = out


ez_pca_scores = data.frame(ez_pca$ind$coord) %>% 
  mutate(sub_id = clean_test_data$sub_id) %>%
  rename( threshold = Dim.1, non_decision = Dim.2, drift_rate = Dim.3) %>%
  filter(sub_id != "s202")
