...
ggsave('EZ_clust_rel.jpeg', device = "jpeg", path = "../output/figures/", width = 6.69, height = 4.36, units = "in", limitsize = FALSE, dpi = 300)
```
...
```{r}
clean_test_data_std %>%
  select(adaptive_n_back.EZ_drift, adaptive_n_back.EZ_non_decision, adaptive_n_back.EZ_thresh, attention_network_task.EZ_drift, attention_network_task.EZ_non_decision, attention_network_task.EZ_thresh) %>%
  gather(dv, value) %>%
  left_join(measure_labels[,c("dv", "rt_acc", "task_group")], by="dv") %>%
  mutate(task_group = ifelse(task_group=="adaptive_n_back", "Task 1", "Task 2")) %>%
  ggplot(aes(value, fill=rt_acc, group=task_group))+
  geom_density(alpha=0.6, color=NA)+
  facet_wrap(~rt_acc+task_group, nrow=1)+
  scale_fill_manual(values=c('#4AC6B7', '#965F8A', '#C61951'))+
  theme(legend.position="none")+
  xlab("")+
  ylab("")

ggsave('t1_ddm_eg.jpeg', device = "jpeg", path = "../output/figures/", width = 12, height = 5, units = "in", limitsize = FALSE, dpi = 300)
```

```{r}
clean_retest_data_std %>%
  select(adaptive_n_back.EZ_drift, adaptive_n_back.EZ_non_decision, adaptive_n_back.EZ_thresh, attention_network_task.EZ_drift, attention_network_task.EZ_non_decision, attention_network_task.EZ_thresh) %>%
  gather(dv, value) %>%
  left_join(measure_labels[,c("dv", "rt_acc", "task_group")], by="dv") %>%
  mutate(task_group = ifelse(task_group=="adaptive_n_back", "Task 1", "Task 2")) %>%
...
ggsave('t2_ddm_eg.jpeg', device = "jpeg", path = "../output/figures/", width = 20, height = 6, units = "in", limitsize = FALSE, dpi = 300)
```