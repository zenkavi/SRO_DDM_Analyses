...
```{r}
ez_t2_fa_3_pred = predict(ez_t1_fa_3, res_clean_retest_data_ez, res_clean_test_data_ez)
...
Comparison of factor scores using T1 EFA model predicting T2 data vs. fitting a new EFA

```{r}
ez_t1_fa_3_scores = data.frame(ez_t1_fa_3$scores)
ez_t1_fa_3_scores = ez_t1_fa_3_scores %>%
  rename(drift_rates = MR1, thresholds = MR2, non_decs = MR3)
ez_t1_fa_3_scores$sub_id = test_data$sub_id

ez_t2_fa_3_pred_scores = data.frame(ez_t2_fa_3_pred)
ez_t2_fa_3_pred_scores = ez_t2_fa_3_pred_scores %>%
  rename(drift_rates = MR1, thresholds = MR2, non_decs = MR3)
ez_t2_fa_3_pred_scores$sub_id = retest_data$sub_id
...
T1_fit scores vs T1_predict_T2 scores

```{r}
ez_t1_fa_3_scores %>%
  left_join(ez_t2_fa_3_pred_scores, by = "sub_id") %>%
  gather(key, value, -sub_id) %>%
  separate(key, c("par", "model"), sep = "\\.") %>%
  mutate(model = ifelse(model=="x", "T1_fit","T2_pred")) %>%
  spread(model, value)%>%
  ggplot(aes(T1_fit, T2_pred))+
  geom_point()+
  facet_wrap(~par, scales= "free")+
  geom_abline(aes(slope=1, intercept=0))
```

T2_fit scores vs T1_predict_T2 scores
...
ez_t2_fa_3_scores %>%
  ...
  gather(key, value, -sub_id) %>%
  separate(key, c("par", "model"), sep = "\\.") %>%
  mutate(model = ifelse(model=="x", "T2_fit","T2_pred")) %>%
  spread(model, value)%>%
  ggplot(aes(T2_fit, T2_pred))+
  geom_point()+
  facet_wrap(~par, scales= "free")+
  geom_abline(aes(slope=1, intercept=0))
```
