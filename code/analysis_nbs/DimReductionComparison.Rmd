...
pca_t1_comp_metrics = find_optimal_components(res_clean_test_data_ez, minc=2, maxc=20, model = "PCA")
```
...
```{r}
dim(res_clean_test_data_ez)
```

```{r warning=FALSE, message=FALSE}
ez_t1_comp_metrics = find_optimal_components(res_clean_test_data_ez, fm = "minres", minc=2)
```
...
```{r}
ez_t1_fa_8 = fa(res_clean_test_data_ez, ez_t1_comp_metrics$comp[1], rotate='oblimin', fm='minres', scores='tenBerge')
```
