...
ez_t1_pca_3 = principal(res_clean_test_data_ez, nfactors=3, rotate="none")
```
...
tmp = ez_t1_pca_3_loadings %>%
  mutate(dv = row.names(.)) %>%
  select(dv, PC1, PC2, PC3) %>%
  mutate(num_loading = 3-(is.na(PC1)+is.na(PC2)+is.na(PC3) ) ) %>%
  filter(num_loading!=0) %>%
  select(-num_loading) %>%
  arrange(-PC1, -PC2, -PC3) %>%
  mutate(order_num = 1:n(),
