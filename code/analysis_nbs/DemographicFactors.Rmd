...
Predicting factor scores for t2 using t1 model doesn't work. There is a problem in matrix dimensions in multiplication. Not sure why.

```{r error=TRUE}
demog_fa_t2_pred = predict(demog_fa_t1, res_demographics_t2)
```

Fitting separate 9 factor solution to t2 data
...
```{r}
data.frame(demog_fa_t2$loadings[]) %>%
  mutate(dv = row.names(.)) %>%
  select(dv, everything()) %>%
  # rename(Drug_Use = ML1,
  #        Daily_Smoking = ML2,
  ...
  #        Mental_Health = ML7,
  #        Problem_Dringking = ML8,
  #        Income_LifeMilestone = ML9) %>%
  rename(Smoking = ML1,
         Mental_Health = ML2,
         Drug_Use = ML3,
         Alcohol_Use = ML4) %>%
...
```{r}
demog_fa_loadings_t1 %>%
  arrange(-Binge_Drinking)
```

```{r}
names(demog_fa_loadings_t1)
```
