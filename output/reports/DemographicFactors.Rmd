...
```{r}
find_optimal_components = function(data, minc=1, maxc=50){
  ...
  for(cur_c in n_components){
    out = psych::fa(data, cur_c, rotate='oblimin', fm='ml', scores='tenBerge')
    if(is.null(out$BIC)){
      curr_metric = NA
    }
    else{
...
```{r}
ian_demog = read.csv('/Users/zeynepenkavi/Dropbox/PoldrackLab/SRO_DDM_Analyses/input/ian_demog_scores.csv')
ian_demog = ian_demog %>%
  rename(sub_id = X,
         Drug_Use = Drug.Use,
